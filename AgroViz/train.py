...
def freeze_layer(trainer):
    model = trainer.model
    print(f"Freezing {config.nb_freezed_layers} layers")
    freeze = [f'model.{x}.' for x in range(config.nb_freezed_layers)]  # layers to freeze
    for k, v in model.named_parameters():
        v.requires_grad = True  # train all layers
        if any(x in k for x in freeze):
            print(f'freezing {k}')
            v.requires_grad = False
    print(f"{config.nb_freezed_layers} layers are freezed.")

if __name__ == '__main__':
    wandb.login(key=config.wandb.get("api_key"))
    wandb.init(project=config.wandb.get("project_name"), 
               notes=f"""Dataset={config.data.get('dataset_name')} 
                // model_size={config.model_size} 
                // lr0={config.train_kwargs.get('lr0')} 
